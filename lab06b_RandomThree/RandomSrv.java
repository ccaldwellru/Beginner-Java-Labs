...
public class RandomSrv {
	...
	public static int[] generateNumbers(int total, int upperBound) {
		Random rand = new Random();
		int[] counter = new int[upperBound];
		for(int i = 0; i < total; i++){
			int currentNum = rand.nextInt(upperBound);
			counter[currentNum]++;
		}
		return counter;
	}
	...
	public static void printMethod(int count[]) {
		for(int i = 0; i < count.length; i++){
			RandomSrv.printMethod(i, count[i]);
		}
	}
}
